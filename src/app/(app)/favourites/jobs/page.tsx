...
export default function FavouriteJobsPage() {
  ...
  const { data, isLoading, isError, error, refetch } = trpc.job.getFavouritedJobs.useQuery({
    ...
    <section className="w-full py-12 md:py-24">
      <div className="container">
        <div className="mx-auto max-w-2xl flex flex-col justify-center items-center">
          <h1 className="text-3xl font-bold tracking-tighter sm:text-5xl">My Favorite Construction Jobs</h1>
          <div className="mt-8 flex flex-col gap-6">
            {isLoading ? <div>Loading...</div> : data?.data.length === 0 ? <div>No favourite jobs found</div> : data?.data.map((job) => (
